(function(window, document) {
  ...
  angular.module('ngS3upload.directives', []).directive('s3Upload', [
    '$parse',
    'S3Uploader',
    'ngS3Config',
    function($parse, S3Uploader, ngS3Config) {
      return {
        restrict: 'AC',
        require: '?ngModel',
        replace: true,
        ...
        controller: [
          ...
          function($scope, $element, $attrs, $transclude) {
            ...
            $scope.barClass = function() {
              return {
                'bar-success':
                  $scope.attempt && !$scope.uploading && $scope.success
              };
            };
          }
        ],
          ...
          return {
            ...
            post: function(scope, element, attrs, ngModel) {
              ...
              var opts = angular.extend(
                {},
                scope.$eval(attrs.s3UploadOptions || attrs.options)
              );
              opts = angular.extend(
                {
                  submitOnChange: true,
              ...
              var uploadFile = function() {
                ...
                S3Uploader.getUploadOptions(opts.getOptionsUri).then(
                  function(s3Options) {
                    ...
                    var key = opts.targetFilename
                      ? scope.$eval(opts.targetFilename)
                      : opts.folder +
                        new Date().getTime() +
                        '-' +
                        S3Uploader.randomString(16) +
                        '.' +
                        ext;
                    S3Uploader.upload(
                      scope,
                  ...
                  function(error) {
                    throw Error(
                      "Can't receive the needed options for S3 " + error
                    );
                  }
                );
              };
  ...
  angular.module('ngS3upload').run([
    '$templateCache',
    function($templateCache) {
      'use strict';

      $templateCache.put(
        'theme/bootstrap2.html',
